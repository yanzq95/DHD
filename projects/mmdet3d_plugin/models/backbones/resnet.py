...
@BACKBONES.register_module()
class MPM_CustomResNet(nn.Module):
    def __init__(
            self,
            numC_input,
            num_layer=[2, 2, 2],
        ...
        super(MPM_CustomResNet, self).__init__()
        # build backbone
        assert len(num_layer) == len(stride)
        num_channels = [numC_input*2**(i+1) for i in range(len(num_layer))] \
            if num_channels is None else num_channels
        ...
        self.layers = nn.Sequential(*layers)
        self.with_cp = with_cp
        self.MPM2D = MPM2D(in_channels=64,ppm_pool_size=(100,50),norm_layer=nn.BatchNorm2d, up_kwargs={'mode': 'nearest'})
